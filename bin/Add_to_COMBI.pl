...
foreach $file (@EEJ){
	...
	if ($eff_href->{$length}{$event} <= 5){ # if 5 or fewer mappable positions, use all counts
	    $new_count = $t[2];
	}
	...
	elsif ($t[2] >= 3 && $positive_pos == 2 && $risky_pos == $positive_pos){ # i.e. 3 or more reads stack into the first or last positions
	    $new_count = 0; # not needed, but to make it explicit
	}
