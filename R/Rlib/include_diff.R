...
parseQual <- function(qual, prior_alpha=1, prior_beta=1) {
  if(is.na(qual) || !grepl("@", qual)) { return(c(1,1)) }  ## for INT NA Columns
...
plotPrint <- function(plotTitle, plotCoord, plotList) {
  ...
  grid.text(plotTitle,gp=gpar(font=2), draw=T, vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
  grid.text(plotCoord,gp=gpar(font=1), draw=T, vp = viewport(layout.pos.row = 2, layout.pos.col = 1:2))
